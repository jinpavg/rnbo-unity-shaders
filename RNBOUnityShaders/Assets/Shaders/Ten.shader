    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float2 V_Remap = i.uv1_uv2 * 2 - 1; // remap from norm to snorm
                float V = (atan2(V_Remap.x, V_Remap.y) / (2 * UNITY_PI)) + 0.5;                 
                V = frac(V * _V_Params.x + _Time.x * _U_Params.y);

                float2 radialUV = float2(U, V);

                fixed4 mask = tex2D(_MainTex, i.uv1_uv2.xy);
