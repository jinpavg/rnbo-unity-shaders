...
public class BufferAnalysisHelper : MonoBehaviour {
    ...
    [DllImport("BufferAnalysis", CallingConvention = CallingConvention.StdCall)]
    ...
    public static BufferAnalysisHelper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("BufferAnalysisHelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                BufferAnalysisHelper helper = go.AddComponent<BufferAnalysisHelper>();
                helper.PluginKey = key;
                return helper;
            }
            return go.GetComponent<BufferAnalysisHelper>();
        }
    }
